    ...
    def CheckforMatchWithPattern(self, Row, Column):
      for StartRow in range(Row + 2, Row - 1, -1):
        for StartColumn in range(Column - 2, Column + 1):
          try:
            ######## Changes start here ########
              if StartColumn < 0:  # wrapping around to the right
                  continue
              # adjust the start column if it's out of bounds
              if StartColumn + 3 > self.__GridSize:
                  StartColumn = self.__GridSize - 3
              ######## Changes end here ########
              PatternString = ""
